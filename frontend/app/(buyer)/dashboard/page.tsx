...
export default function BuyerDashboard() {
  usePageTitle("Dashboard", "SupplySync Buyer");
    ...
    const fetchData = async () => {
      try {
        ...
        const activeCount = orders.filter((o: Order) =>
          ["PENDING", "MATCHED", "DISPATCHED", "IN_TRANSIT"].includes(o.status),
        ).length;
          ...
          deliveredOrders.length > 0
            ? Math.round(
                deliveredOrders.reduce(
                  (sum: number, o: Order) =>
                    sum + (o.estimated_delivery_minutes || 0),
                  0,
        ...
        const uniqueSuppliers = new Set(
          orders.map((o: Order) => o.matched_supplier?.id),
        ).size;
            ...
            <div key={i} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="h-7 bg-slate-300 rounded w-12" />
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/orders/new"
          className="px-4 py-2 bg-primary text-white rounded-md text-sm font-medium hover:bg-primary/90 transition flex items-center shadow-sm"
        >
          Place New Order
        </Link>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-sm">
                ...
                <th className="px-6 py-3 text-slate-500 font-mediumtext-right">
                  Action
                </th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="px-6 py-4 flex gap-2">
                    <Link
                      ...
                      className="text-primary hover:text-primary/70 text-sm font-medium flex items-center gap-1 transition"
                    >
