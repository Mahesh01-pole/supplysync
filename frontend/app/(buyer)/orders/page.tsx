...
const getStatusBadge = (status: string) => {
  ...
  return (
    <span
      className={`px-2 py-1 text-xs font-medium rounded-full flex items-center gap-1 w-fit ${cfg[status] ?? "bg-slate-100 text-slate-800"}`}
    >
      {status === "IN_TRANSIT" && (
...
export default function OrdersListPage() {
  usePageTitle("My Orders", "SupplySync Buyer");
  ...
  const filtered = orders.filter((o) => {
    ...
    const matchSearch =
      ...
      (o.matched_supplier?.company_name ?? "").toLowerCase().includes(q);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Track and manage all your supply requests.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/orders/new"
            className="px-4 py-2 bg-primary text-white rounded-md text-sm font-medium hover:bg-primary/90 transition shadow-sm"
          >
            New Request
          </Link>
        </div>
        ...
        {loading ? (
          <div className="animate-pulse">
            <div className="p-4 border-b border-slate-200 flex gap-3 bg-slate-50">
              <div className="h-9 bg-slate-200 rounded flex-1 max-w-md" />
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3">
            ...
            <p className="font-medium">
              {orders.length === 0
                ? "No orders yet. Place your first request!"
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link
                        ...
                        className="inline-flex items-center justify-center p-2 rounded-full hover:bg-slate-200 text-slate-400 group-hover:text-primary transition"
                      >
                        <ArrowRight size={18} />
                      </Link>
                    </td>
