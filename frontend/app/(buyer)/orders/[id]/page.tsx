...
const LiveTrackingMap = dynamic(
  ...
  { ssr: false, loading: () => <div className="w-full h-full bg-slate-100 rounded-xl flex items-center justify-center"><Loader2 className="animate-spin text-slate-400" size={28} /></div> }
);
...
export default function OrderDetailsPage({
  ...
  usePageTitle(order ? `Order ${order.order_number}` : "Order Details", "SupplySync Buyer");
  ...
  const getUrgencyBadge = (u: string) => {
    if (u === "P1")
      return (
        <span className="px-3 py-1.5 bg-red-50 text-red-700 text-sm font-bold rounded-md border border-red-100 flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-red-600 animate-pulse" /> P1
          Critical
        </span>
      );
    if (u === "P2")
      return (
        <span className="px-3 py-1.5 bg-amber-50 text-amber-700 text-sm font-bold rounded-md border border-amber-100 flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-amber-500" /> P2 Urgent
        </span>
      );
    return (
      <span className="px-3 py-1.5 bg-slate-50 text-slate-700 text-sm font-bold rounded-md border border-slate-200 flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-slate-400" /> P3 Standard
      </span>
    );
  };
  ...
  const timeline = [
    ...
    { state: "DISPATCHED", label: "Dispatched", time: order?.dispatched_at },
    {
      state: "IN_TRANSIT",
      label: "In Transit",
      time: order?.status === "IN_TRANSIT" ? "Live" : null,
    },
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <Link
        href="/orders"
        className="text-sm text-slate-500 hover:text-primary flex items-center gap-1 w-fit transition"
      >
              ...
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-bold text-slate-900">
                    {order.matched_supplier.company_name}
                  </p>
                  <div className="flex items-center gap-1 text-sm text-slate-500 mt-1">
                    <Star size={14} className="text-amber-500 fill-amber-500" />
                    <span className="font-medium text-slate-700">
                      {Number(order.matched_supplier.rating).toFixed(1)}
                    </span>
            ...
            <div className="space-y-6">
              {timeline.map((step, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex gap-4 relative ${isActive ? "text-slate-900" : "text-slate-400"}`}
                  >
                    {i !== timeline.length - 1 && (
                      <div
                        className={`absolute left-3.5 top-8 w-0.5 h-10 ${isActive && i + 1 <= activeIdx ? "bg-indigo-600" : "bg-slate-200"}`}
                      />
                    )}
                    <div
                      className={`w-8 h-8 rounded-full flex justify-center items-center flex-shrink-0 z-10 ${isActive ? "bg-indigo-100 text-indigo-600" : "bg-slate-100 text-slate-300"}`}
                    >
                      {isActive ? (
                    ...
                    <div>
                      ...
                      <p
                        className={`text-xs mt-1 ${isActive ? "text-slate-500" : "text-slate-400"}`}
                      >
                        {step.time === "Live" ? (
