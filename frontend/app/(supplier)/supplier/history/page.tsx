...
export default function SupplierHistory() {
  ...
  const load = useCallback(async () => {
    ...
    // Resolve supplierId: try cookie first, then /api/auth/me
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/suppliers/${supplierId}/history`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      setHistory(items);
      setTotalEarnings(
        items.reduce((s, i) => s + (Number(i.earnings) || 0), 0),
      );
    } catch {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            A record of your completed fulfillments and earnings.
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-sm">
                  ...
                  <th className="px-6 py-4 text-slate-500 font-medium text-center">
                    Fulfillment
                  </th>
                  <th className="px-6 py-4 text-slate-500 font-medium text-center">
                    Rating
                  </th>
                  <th className="px-6 py-4 text-slate-500 font-medium text-right">
                    Earnings (₹)
                  </th>
                </tr>
              </thead>
              ...
              <tfoot>
                <tr className="bg-emerald-50 border-t-2 border-emerald-200">
                  <td
                    colSpan={4}
                    className="px-6 py-3 text-sm font-bold text-emerald-800"
                  >
                    Total Earnings
                  </td>
