      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.weeklyVolume) {
            setLineData((prev) => 
              prev.map(day => {
                const found = data.weeklyVolume.find((d: any) => d.name === day.name);
                return found ? { ...day, orders: found.orders } : day;
              })
            );
          }
  ...
  const generateInsights = async () => {
    ...
    try {
      ...
      if (data.insights && Array.isArray(data.insights)) {
        ...
        toast.success("AI Insights refreshed via Groq API");
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500">
          System-wide performance metrics and Groq-generated recommendations.
        </p>
      </div>
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold flex items-center gap-2">
              ...
              Groq AI Insights (Llama 3)
            </h2>
            <button
              onClick={generateInsights}
              disabled={isGenerating}
              className="bg-white text-indigo-900 hover:bg-slate-50 transition px-3 py-1.5 rounded-md text-sm font-bold flex items-center gap-2 disabled:opacity-50 shadow-sm"
            >
              <RefreshCw
                size={14}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm lg:col-span-2">
          <h3 className="font-bold text-slate-900 mb-6">
            Real-Time Global Status Distribution
          </h3>
