...
export default function AdminOrdersPage() {
  usePageTitle("All Orders", "SupplySync Admin");
  ...
  const filtered = orders.filter((o) => {
    ...
    const ms = (
      ...
      o.product.name +
      (o.buyer?.company ?? "") +
      (o.matched_supplier?.company_name ?? "")
    ).toLowerCase();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Every order across all buyers and suppliers.
          </p>
        </div>
        <button
          onClick={load}
              ...
              <tbody className="divide-y divide-slate-100">
                {[...Array(8)].map((_, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-5 py-4"><div className="h-3 bg-slate-200 rounded w-28" /></td>
                    <td className="px-5 py-4 text-right"><div className="h-3 bg-slate-100 rounded w-10 ml-auto" /></td>
                    <td className="px-5 py-4"><div className="h-5 bg-slate-200 rounded-full w-20" /></td>
                    <td className="px-5 py-4"><div className="h-3 bg-slate-100 rounded w-20" /></td>
                    <td className="px-5 py-4 text-right"><div className="h-6 bg-slate-200 rounded-lg w-14 ml-auto" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                {[
                  ...
                  <th
                    key={h}
                    className={`px-5 py-3.5 font-semibold text-slate-600 ${h === "Qty" || h === "" ? "text-right" : "text-left"}`}
                  >
                    {h}
                  </th>
                ))}
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-right">
                        <Link
                          ...
                          className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-purple-100 hover:text-purple-700 text-slate-600 text-xs font-medium transition-colors"
                        >
                          <Eye size={13} /> View
                        </Link>
                      </td>
